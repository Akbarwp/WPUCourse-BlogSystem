<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            <x-input-label for="userCred" :value="__('Email or Username')" />
            <x-text-input id="userCred" class="block mt-1 w-full" type="text" name="userCred" :value="old('userCred')" required autofocus autocomplete="userCred" />
            <x-input-error :messages="$errors->get('userCred')" class="mt-2" />
        </div>
