                    ...
                    <form action="{{ route("category.update", $category->slug) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="w-full flex flex-col md:flex-row justify-center items-center gap-2">
                            <fieldset class="fieldset w-full md:w-1/2">
                                ...
                                <input type="text" name="name" class="input @error("name") input-error bg-red-50 @enderror w-full" placeholder="Lorem ipsum dolor sit amet" value="{{ old("name") ?? $category->name }}" autofocus required />
                                @error("name")
                            ...
                            <fieldset class="fieldset w-full md:w-1/2">
                                ...
                                <select class="select @error("color") select-error @enderror w-full" name="color" required>
                                    ...
                                    @foreach ($colors as $color)
                                        <option value="{{ 'bg-'.$color.'-100' }}" @selected(old("color") ?? $category->color == ('bg-'.$color.'-100')) class="{{ 'bg-'.$color.'-100' }}">{{ $color }}</option>
                                    @endforeach
                                </select>
                                @error("color")
                                    <p class="label text-error">{{ $message }}</p>
